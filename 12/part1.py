...
def dijkstra_algorithm(data, start_node):
    unvisited_nodes = set([(x, y) for x in range(len(data))
...
def main():
    data = np.genfromtxt(
        "/mnt/c/Users/felix/workspace/adventOfCode/12/in.txt", delimiter=1, dtype=str)
    start_pos = list(zip(*np.where(data == "S")))[0]
    end_pos = list(zip(*np.where(data == "E")))[0]
    data[end_pos] = "z"
    data[start_pos] = "a"
    print(dijkstra_algorithm(data, start_pos)[1][end_pos])
